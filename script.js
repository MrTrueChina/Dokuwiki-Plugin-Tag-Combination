...
jQuery(document).ready(() => {
    ...
    if (JSINFO && JSINFO.act === 'show') {
        ...
        jQuery(document).keypress((e) => {
            ...
            let configKey = JSINFO.tagcombination.config.shortcutKey;
            let needCtrlKey = JSINFO.tagcombination.config.needCtrlKey;
            let needAltKey = JSINFO.tagcombination.config.needAltKey;
            ...
            if (key && ctrlKey && altKey) {
                // 打开搜索页面，就是在当前网址后面拼上 plugin_tagcombination__showsearchpage
                location.replace(window.location.href + '&do=plugin_tagcombination__showsearchpage');
            }
        });
    }
});
